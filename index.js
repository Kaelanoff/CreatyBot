...
const CHANNEL_SECTIONS = {
  aeroport: { label: 'Aéroport', keys: ['bienvenue','depart'] },
  hub: { label: 'Hub', keys: ['reglement','annonces','info','roadmap','liens','faq','sondages'] },
  support: { label: 'Support', keys: ['ticket','attente_vocale','support_premium'] },
  communaute: { label: 'Communauté', keys: ['discussion','media','suggestion','vos_bots','presentation','evenements'] },
  services: { label: 'Nos Services', keys: ['creation_bot','creation_serveur','hebergement','tarifs','garantie'] },
  commandes: { label: 'Commandes', keys: ['commander','demander_devis','suivi_commandes','paiements','conditions','questions_commandes','offres_speciales'] },
  espace_client: { label: 'Espace Client', keys: ['infos_clients','livraisons_clients','factures','avis'] },
  client_premium: { label: 'Client Premium', keys: ['support_premium','commandes_premium','avantages_premium','annonces_premium','premium_chat'] },
  developpement: { label: 'Développement', keys: ['annonces_dev','discussion_dev','documentation','tests_dev','bugs'] },
  projets_clients: { label: 'Projets Clients', keys: ['liste_projets','projets_attente','analyse','developpement','tests_projets','corrections','termines','livraisons_projets','archives'] },
  commercial: { label: 'Commercial', keys: ['ventes','devis_commerciaux','commandes_commerciales','statistiques_commerciales','objectifs','chiffre_affaires','discussion_commerciale'] },
  design: { label: 'Design', keys: ['creations','logos','bannieres','miniatures','reseaux_sociaux','discussion_design'] },
  staff: { label: 'Staff', keys: ['staff_chat','staff_annonces','recrutements','sanctions','reunions'] },
  direction: { label: 'Direction', keys: ['direction','finance','statistiques_globales','planning','partenaires','contrats','decisions','documents'] },
  fondation: { label: 'Fondation', keys: ['fondation','documents_confidentiels','projets_secrets','gestion_financiere','acces_total','journal_direction'] },
  gestion_clients: { label: 'Gestion Clients', keys: ['liste_clients','nouveaux_clients','clients_premium'] },
  technique: { label: 'Technique', keys: ['logs_bot','logs_erreurs','logs_roles'] }
};
...
async function smartReply(interaction, payload) {
  ...
  return smartReply(interaction, payload);
}
...
async function showTracking(interaction,type){const db=getDb(),gid=interaction.guildId||interaction.guild?.id;let items=[];if(type==='quotes')items=Object.values(db.quotes).filter(x=>x.guildId===gid&&x.userId===interaction.user.id);if(type==='orders')items=Object.values(db.orders).filter(x=>x.guildId===gid&&x.userId===interaction.user.id);if(type==='projects')items=Object.values(db.projects).filter(x=>x.guildId===gid&&x.userId===interaction.user.id);if(!items.length)return smartReply(interaction, {content:'Aucun élément enregistré pour toi.',flags:MessageFlags.Ephemeral});let desc='';if(type==='quotes')desc=items.map(x=>`**${x.id} — ${x.projectName}**\n${x.status} • ${x.price==null?'Prix à définir':`${x.price} €`}`).join('\n\n');if(type==='orders')desc=items.map(x=>`**${x.id} — ${x.projectName}**\n${x.status} • Paiement : ${x.paymentStatus}`).join('\n\n');if(type==='projects')desc=items.map(x=>{const s=PROJECT_STAGES[x.stageIndex];return `**${x.id} — ${x.projectName}**\n${s.emoji} ${s.label} • ${s.progress}%`}).join('\n\n');return smartReply(interaction, {embeds:[embed(type==='quotes'?'💰 Mes devis':type==='orders'?'📦 Mes commandes':'🚀 Mes projets',desc.slice(0,3900))],flags:MessageFlags.Ephemeral})}

const commands=[
  new SlashCommandBuilder().setName('config').setDescription('Configuration complète.')
    .addSubcommand(s=>s.setName('salon').setDescription('Configurer un salon.').addStringOption(o=>o.setName('section').setDescription('Section du serveur').setRequired(true).addChoices(...Object.entries(CHANNEL_SECTIONS).map(([value,v])=>({name:v.label,value})))).addStringOption(o=>o.setName('type').setDescription('Salon à configurer').setRequired(true).setAutocomplete(true)).addChannelOption(o=>o.setName('cible').setDescription('Salon Discord').setRequired(true).addChannelTypes(ChannelType.GuildText,ChannelType.GuildAnnouncement,ChannelType.GuildForum,ChannelType.GuildVoice,ChannelType.GuildStageVoice)))
    .addSubcommand(s=>s.setName('categorie').setDescription('Configurer une catégorie.').addStringOption(o=>o.setName('type').setDescription('Catégorie à configurer').setRequired(true).setAutocomplete(true)).addChannelOption(o=>o.setName('cible').setDescription('Catégorie Discord').setRequired(true).addChannelTypes(ChannelType.GuildCategory)))
...
client.on(Events.InteractionCreate,async interaction=>{
  try{
    if(interaction.isAutocomplete()){
      ...
      if(interaction.commandName==='config'&&focused.name==='type'){
        ...
        if(sub==='salon'){
          ...
          const keys=CHANNEL_SECTIONS[section]?.keys||[];
          choices=CHANNELS.filter(([k])=>keys.includes(k));
        } else if(sub==='categorie') choices=CATEGORIES;
        else if(sub==='role') choices=ROLES;
    ...
    if(interaction.isChatInputCommand()){
      ...
      if(!guild)return smartReply(interaction, {content:'Commande disponible uniquement sur un serveur.',flags:MessageFlags.Ephemeral});
      ...
      if(interaction.commandName==='config'){
        ...
        const sub=interaction.options.getSubcommand();
        if(sub==='salon'){const type=interaction.options.getString('type'),target=interaction.options.getChannel('cible');if(!CHANNELS.some(([k])=>k===type))return smartReply(interaction,{content:'❌ Salon de configuration inconnu.',flags:MessageFlags.Ephemeral});setConfig(guild.id,'channels',type,target.id);await journal(guild.id,`Configuration salon ${type} → ${target.id}`);return smartReply(interaction, {content:`✅ Salon **${CHANNELS.find(([k])=>k===type)?.[1]||type}** configuré sur ${target}.`,flags:MessageFlags.Ephemeral})}
        if(sub==='categorie'){const type=interaction.options.getString('type'),target=interaction.options.getChannel('cible');if(!CATEGORIES.some(([k])=>k===type))return smartReply(interaction,{content:'❌ Catégorie de configuration inconnue.',flags:MessageFlags.Ephemeral});setConfig(guild.id,'categories',type,target.id);await journal(guild.id,`Configuration catégorie ${type} → ${target.id}`);return smartReply(interaction,{content:`✅ Catégorie **${CATEGORIES.find(([k])=>k===type)?.[1]||type}** configurée sur ${target}.`,flags:MessageFlags.Ephemeral})}
        if(sub==='role'){const type=interaction.options.getString('type'),role=interaction.options.getRole('role');setConfig(guild.id,'roles',type,role.id);await journal(guild.id,`Configuration rôle ${type} → ${role.id}`);if(['fondateur','cofondateur','directeur','directeur_general'].includes(type))await safeLog(guild.id,'acces_total',{content:`⚠️ Configuration sensible : rôle **${type}** modifié par <@${interaction.user.id}>.`});return smartReply(interaction, {content:`✅ ${type} configuré sur ${role}.`,flags:MessageFlags.Ephemeral})}
        if(sub==='paiement'){setConfig(guild.id,'settings','paypalUrl',interaction.options.getString('lien'));setConfig(guild.id,'settings','paymentMessage',interaction.options.getString('message')||'Utilise le lien PayPal officiel ci-dessous.');await safeLog(guild.id,'acces_total',{content:`⚠️ Lien PayPal modifié par <@${interaction.user.id}>.`});return smartReply(interaction, {content:'✅ Paiement PayPal configuré.',flags:MessageFlags.Ephemeral})}
      ...
      if(interaction.commandName==='tarif'){if(!hasFeature(interaction.member,c,'tarifs'))return smartReply(interaction, {content:'❌ Non autorisé.',flags:MessageFlags.Ephemeral});const ch=await guild.channels.fetch(c.channels.tarifs).catch(()=>null);if(!ch?.isTextBased())return smartReply(interaction, {content:'❌ Salon Tarifs non configuré.',flags:MessageFlags.Ephemeral});await upsertPanel(guild,'tarifs');return smartReply(interaction, {content:'✅ Tarifs publiés/mis à jour.',flags:MessageFlags.Ephemeral})}
      if(interaction.commandName==='liens'){if(!hasFeature(interaction.member,c,'liens'))return smartReply(interaction, {content:'❌ Non autorisé.',flags:MessageFlags.Ephemeral});setConfig(guild.id,'settings','linksText',interaction.options.getString('contenu'));await upsertPanel(guild,'liens');return smartReply(interaction, {content:'✅ Panneau Nos liens mis à jour sans doublon.',flags:MessageFlags.Ephemeral})}
      if(interaction.commandName==='role'){if(!isAdmin(interaction.member)&&!configuredRoleIds(c,['fondateur','cofondateur','directeur_general','directeur']).some(id=>interaction.member.roles.cache.has(id)))return smartReply(interaction, {content:'❌ Non autorisé.',flags:MessageFlags.Ephemeral});const user=interaction.options.getUser('utilisateur'),grade=interaction.options.getString('grade'),member=await getMember(guild,user.id);if(!member)return smartReply(interaction, {content:'❌ Utilisateur introuvable.',flags:MessageFlags.Ephemeral});if(interaction.options.getSubcommand()==='attribuer')await assignGrade(member,grade,c);else await removeGrade(member,grade,c);return smartReply(interaction, {content:'✅ Modification de rôle confirmée par Discord.',flags:MessageFlags.Ephemeral})}
      if(interaction.commandName==='projet'){if(!isStaff(interaction.member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});const id=interaction.options.getString('id').toUpperCase(),db=getDb(),p=db.projects[id];if(!p)return smartReply(interaction, {content:'❌ Projet introuvable.',flags:MessageFlags.Ephemeral});const sub=interaction.options.getSubcommand();let idx=p.stageIndex;if(sub==='avancer')idx=Math.min(PROJECT_STAGES.length-1,idx+1);if(sub==='reculer')idx=Math.max(0,idx-1);if(sub==='etape')idx=Number(interaction.options.getString('etape'));const changed=await changeProjectStage(guild,p,idx);return smartReply(interaction, {content:changed?'✅ Étape modifiée.':'ℹ️ Le projet est déjà à cette étape.',flags:MessageFlags.Ephemeral})}
      if(interaction.commandName==='suivi'){return showTracking(interaction,'projects')}
      if(interaction.commandName==='sondage'){if(!hasFeature(interaction.member,c,'sondages'))return smartReply(interaction, {content:'❌ Non autorisé.',flags:MessageFlags.Ephemeral});const ch=await guild.channels.fetch(c.channels.sondages).catch(()=>null);if(!ch?.isTextBased())return smartReply(interaction, {content:'❌ Salon Sondages non configuré.',flags:MessageFlags.Ephemeral});const q=interaction.options.getString('question'),answers=interaction.options.getString('reponses').split('|').map(x=>x.trim()).filter(Boolean).slice(0,10),type=interaction.options.getString('type'),duration=interaction.options.getInteger('duree');if(answers.length<2)return smartReply(interaction, {content:'❌ Il faut au moins 2 réponses séparées par |.',flags:MessageFlags.Ephemeral});const pollId=`poll-${Date.now()}`;const menu=new StringSelectMenuBuilder().setCustomId(`poll_vote:${pollId}:${type}`).setPlaceholder(type==='multiple'?'Choisis une ou plusieurs réponses':'Choisis une réponse').setMinValues(1).setMaxValues(type==='multiple'?answers.length:1).addOptions(answers.map((a,i)=>({label:a.slice(0,100),value:String(i)})));const m=await ch.send({embeds:[embed('📊 Sondage',`**${q}**\n${duration?`Durée : ${duration} min`:''}`)],components:[new ActionRowBuilder().addComponents(menu)]});const db=getDb();db.polls[pollId]={id:pollId,guildId:guild.id,messageId:m.id,channelId:ch.id,question:q,answers,votes:{},endsAt:duration?Date.now()+duration*60000:null};writeJson(DB_FILE,db);return smartReply(interaction, {content:'✅ Sondage publié.',flags:MessageFlags.Ephemeral})}
      if(interaction.commandName==='offre'){if(!hasFeature(interaction.member,c,'offres'))return smartReply(interaction, {content:'❌ Non autorisé.',flags:MessageFlags.Ephemeral});const ch=await guild.channels.fetch(c.channels.offres_speciales).catch(()=>null);if(!ch?.isTextBased())return smartReply(interaction, {content:'❌ Salon Offres spéciales non configuré.',flags:MessageFlags.Ephemeral});const id=nextId('offers','OFFRE'),o={id,guildId:guild.id,title:interaction.options.getString('titre'),description:interaction.options.getString('description'),offer:interaction.options.getString('offre'),initial:interaction.options.getNumber('prix_initial'),promo:interaction.options.getNumber('prix_promo'),end:interaction.options.getString('fin'),quantity:interaction.options.getInteger('quantite'),createdAt:new Date().toISOString()};const db=getDb();db.offers[id]=o;writeJson(DB_FILE,db);await ch.send({embeds:[embed(`🔥 ${o.title}`,`${o.description}\n\nOffre : **${o.offer}**\nPrix initial : ~~${o.initial.toFixed(2)} €~~\nPrix promo : **${o.promo.toFixed(2)} €**${o.end?`\nFin : **${o.end}**`:''}${o.quantity?`\nQuantité : **${o.quantity}**`:''}`,0xED4245)]});return smartReply(interaction, {content:`✅ Offre ${id} publiée.`,flags:MessageFlags.Ephemeral})}
      if(interaction.commandName==='design'){if(!isStaff(interaction.member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});const id=nextId('designs','DESIGN'),d={id,guildId:guild.id,type:interaction.options.getString('type'),title:interaction.options.getString('titre'),description:interaction.options.getString('description'),userId:interaction.options.getUser('client')?.id||null,claimedBy:interaction.user.id,status:'À faire',createdAt:new Date().toISOString()};const db=getDb();db.designs[id]=d;writeJson(DB_FILE,db);for(const key of ['creations',d.type]){const ch=await guild.channels.fetch(c.channels[key]).catch(()=>null);if(ch?.isTextBased())await ch.send({embeds:[designEmbed(d)],components:designControls(d)})}return smartReply(interaction, {content:`✅ Projet design ${id} créé.`,flags:MessageFlags.Ephemeral})}
      if(['tache','partenaire','contrat','decision'].includes(interaction.commandName)){if(!isStaff(interaction.member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});const map={tache:['tasks','TACHE','planning'],partenaire:['partners','PART','partenaires'],contrat:['contracts','CONTRAT','contrats'],decision:['decisions','DEC','decisions']};const [collection,prefix,channelKey]=map[interaction.commandName],id=nextId(collection,prefix);let item={id,guildId:guild.id,createdBy:interaction.user.id,createdAt:new Date().toISOString()};if(interaction.commandName==='tache')item={...item,title:interaction.options.getString('titre'),responsibleId:interaction.options.getUser('responsable').id,priority:interaction.options.getString('priorite'),deadline:interaction.options.getString('echeance'),status:'À faire'};else if(interaction.commandName==='partenaire')item={...item,name:interaction.options.getString('nom'),details:interaction.options.getString('details')};else item={...item,title:interaction.options.getString('titre'),details:interaction.options.getString('details')};const db=getDb();db[collection][id]=item;writeJson(DB_FILE,db);const ch=await guild.channels.fetch(c.channels[channelKey]).catch(()=>null);if(ch?.isTextBased())await ch.send({embeds:[embed(id,interaction.commandName==='tache'?`Titre : **${item.title}**\nResponsable : <@${item.responsibleId}>\nPriorité : **${item.priority}**\nÉchéance : **${item.deadline}**\nStatut : **${item.status}**`:interaction.commandName==='partenaire'?`Nom : **${item.name}**\n${item.details}`:`Titre : **${item.title}**\n${item.details}`)]});return smartReply(interaction, {content:`✅ ${id} créé.`,flags:MessageFlags.Ephemeral})}
    }
    ...
    if(interaction.isStringSelectMenu()){
      ...
      if(interaction.customId==='ticket_select'||interaction.customId==='ticket_select_premium'){const ch=await createTicket(interaction.guild,interaction.user.id,interaction.values[0],interaction.customId.endsWith('premium'));return smartReply(interaction, {content:`✅ Ticket créé : ${ch}.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId==='faq_select'){const val=interaction.values[0];const components=val==='q19'?[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('go_support').setLabel('Ouvrir un ticket').setStyle(ButtonStyle.Primary))]:[];return smartReply(interaction, {embeds:[embed('❓ FAQ',faqAnswer(val))],components,flags:MessageFlags.Ephemeral})}
      if(interaction.customId==='order_question_select'){const ch=await createTicket(interaction.guild,interaction.user.id,`commande-${interaction.values[0]}`);return smartReply(interaction, {content:`✅ Ticket créé : ${ch}.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId.startsWith('project_stage:')){const id=interaction.customId.split(':')[1],db=getDb(),p=db.projects[id],c=getConfig(p?.guildId);if(!p)return smartReply(interaction, {content:'❌ Projet introuvable.',flags:MessageFlags.Ephemeral});if(!isStaff(interaction.member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});const changed=await changeProjectStage(interaction.guild,p,Number(interaction.values[0]));return smartReply(interaction, {content:changed?'✅ Étape modifiée.':'ℹ️ Étape inchangée.',flags:MessageFlags.Ephemeral})}
      if(interaction.customId.startsWith('design_stage:')){const id=interaction.customId.split(':')[1],db=getDb(),d=db.designs[id];if(!d)return smartReply(interaction, {content:'❌ Projet design introuvable.',flags:MessageFlags.Ephemeral});d.status=interaction.values[0];db.designs[id]=d;writeJson(DB_FILE,db);return interaction.update({embeds:[designEmbed(d)],components:designControls(d)})}
      if(interaction.customId.startsWith('poll_vote:')){const[,id]=interaction.customId.split(':');const db=getDb(),p=db.polls[id];if(!p)return smartReply(interaction, {content:'❌ Sondage introuvable.',flags:MessageFlags.Ephemeral});if(p.endsAt&&Date.now()>p.endsAt)return smartReply(interaction, {content:'❌ Ce sondage est terminé.',flags:MessageFlags.Ephemeral});p.votes[interaction.user.id]=interaction.values;db.polls[id]=p;writeJson(DB_FILE,db);return smartReply(interaction, {content:'✅ Vote enregistré.',flags:MessageFlags.Ephemeral})}
    }

    if(interaction.isButton()){
      if(interaction.customId==='rules_accept'){const guild=interaction.guild,c=getConfig(guild.id),member=interaction.member;try{if(c.roles.nouveau&&member.roles.cache.has(c.roles.nouveau))await applyRole(member,c.roles.nouveau,'remove',guild.id,'Nouveau');await applyRole(member,c.roles.membre,'add',guild.id,'Membre');return smartReply(interaction, {content:`✅ Règlement accepté.\nLe rôle <@&${c.roles.membre}> vous a été attribué.`,flags:MessageFlags.Ephemeral})}catch(e){await logError(guild.id,e,'Acceptation règlement');return smartReply(interaction, {content:`❌ Impossible de finaliser l’acceptation : ${e.message}`,flags:MessageFlags.Ephemeral})}}
      ...
      if(interaction.customId==='go_order')return smartReply(interaction, {content:'Utilise le panneau **Commander** pour choisir ton offre.',flags:MessageFlags.Ephemeral});
      if(interaction.customId==='go_support'){const ch=await createTicket(interaction.guild,interaction.user.id,'support');return smartReply(interaction, {content:`✅ Ticket créé : ${ch}.`,flags:MessageFlags.Ephemeral})}
      ...
      if(interaction.customId==='review_open'){const db=getDb(),gid=interaction.guild.id;const ok=Object.values(db.projects).some(p=>p.guildId===gid&&p.userId===interaction.user.id&&PROJECT_STAGES[p.stageIndex]?.key==='archives');if(!ok)return smartReply(interaction, {content:'❌ Vous devez avoir terminé au moins une commande pour laisser un avis.',flags:MessageFlags.Ephemeral});const modal=new ModalBuilder().setCustomId('review_modal').setTitle('Laisser un avis');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('title').setLabel('Titre').setStyle(TextInputStyle.Short).setRequired(true)),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('rating').setLabel('Note sur 5').setStyle(TextInputStyle.Short).setRequired(true)),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('message').setLabel('Message').setStyle(TextInputStyle.Paragraph).setRequired(true)));return interaction.showModal(modal)}
      ...
      if(interaction.customId==='test_mine'){const arr=Object.values(getDb().tests).filter(x=>x.guildId===interaction.guild.id&&x.userId===interaction.user.id);return smartReply(interaction, {embeds:[embed('🧪 Mes tests',arr.length?arr.map(x=>`**${x.id}** — ${x.projectId} — ${x.status}`).join('\n'):'Aucun test.')],flags:MessageFlags.Ephemeral})}
      ...
      if(interaction.customId==='bug_mine'){const arr=Object.values(getDb().bugs).filter(x=>x.guildId===interaction.guild.id&&x.userId===interaction.user.id);return smartReply(interaction, {embeds:[embed('🐞 Mes bugs',arr.length?arr.map(x=>`**${x.id}** — ${x.projectId} — ${x.status}`).join('\n'):'Aucun bug.')],flags:MessageFlags.Ephemeral})}
      ...
      if(interaction.customId.startsWith('quote_')){const[action,id]=interaction.customId.split(':'),db=getDb(),q=db.quotes[id];if(!q)return smartReply(interaction, {content:'❌ Devis introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(q.guildId),c=getConfig(q.guildId),member=await getMember(guild,interaction.user.id);if(action==='quote_client_accept'||action==='quote_client_refuse'){if(interaction.user.id!==q.userId)return smartReply(interaction, {content:'❌ Ce devis ne vous appartient pas.',flags:MessageFlags.Ephemeral});q.status=action==='quote_client_accept'?'Accepté par le client':'Refusé';db.quotes[id]=q;writeJson(DB_FILE,db);await saveQuoteCard(guild,q);return smartReply(interaction, {content:`✅ Devis ${q.status.toLowerCase()}.`,flags:MessageFlags.Ephemeral})}if(!isStaff(member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});if(action==='quote_claim'){q.claimedBy=interaction.user.id;q.status='Pris en charge'}else if(action==='quote_price'){const modal=new ModalBuilder().setCustomId(`quote_price_modal:${id}`).setTitle('Définir le prix');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('price').setLabel('Prix en €').setStyle(TextInputStyle.Short).setRequired(true)));return interaction.showModal(modal)}else if(action==='quote_send'){if(q.price==null)return smartReply(interaction, {content:'❌ Définissez d’abord le prix.',flags:MessageFlags.Ephemeral});const ok=await sendDm(q.userId,{embeds:[quoteEmbed(q)],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`quote_client_accept:${id}`).setLabel('Accepter').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId(`quote_client_refuse:${id}`).setLabel('Refuser').setStyle(ButtonStyle.Danger))]});if(!ok)return smartReply(interaction, {content:'❌ MP impossible.',flags:MessageFlags.Ephemeral});q.status='Envoyé au client'}else if(action==='quote_contact'){const modal=new ModalBuilder().setCustomId(`contact_modal:quotes:${id}`).setTitle('Contacter le client');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('text').setLabel('Message').setStyle(TextInputStyle.Paragraph).setRequired(true)));return interaction.showModal(modal)}else if(action==='quote_order'){if(q.status!=='Accepté par le client')return smartReply(interaction, {content:'❌ Le client doit accepter le devis.',flags:MessageFlags.Ephemeral});const o=await createOrder(guild,q.userId,'custom',q.projectName,q.service,q.description,q.price,q.id);q.status='Transformé en commande';q.orderId=o.id}else if(action==='quote_refuse')q.status='Refusé';else if(action==='quote_archive')q.status='Archivé';db.quotes[id]=q;writeJson(DB_FILE,db);await saveQuoteCard(guild,q);return smartReply(interaction, {content:`✅ ${q.status}.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId.startsWith('order_')){const[action,id]=interaction.customId.split(':'),db=getDb(),o=db.orders[id];if(!o)return smartReply(interaction, {content:'❌ Commande introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(o.guildId),c=getConfig(o.guildId),member=await getMember(guild,interaction.user.id);if(!isStaff(member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});if(action==='order_claim'){o.claimedBy=interaction.user.id;o.status='Prise en charge'}else if(action==='order_accept')o.status='Acceptée';else if(action==='order_refuse')o.status='Refusée';else if(action==='order_contact'){const modal=new ModalBuilder().setCustomId(`contact_modal:orders:${id}`).setTitle('Contacter le client');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('text').setLabel('Message').setStyle(TextInputStyle.Paragraph).setRequired(true)));return interaction.showModal(modal)}else if(action==='order_pay'){if(!c.settings.paypalUrl)return smartReply(interaction, {content:'❌ PayPal non configuré.',flags:MessageFlags.Ephemeral});const ok=await sendDm(o.userId,{embeds:[embed(`💳 Paiement — ${o.id}`,`${c.settings.paymentMessage||''}\n\nMontant : **${o.price==null?'À définir':`${Number(o.price).toFixed(2)} €`}**\nLien officiel : ${c.settings.paypalUrl}`)],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`payment_from_order:${o.id}`).setLabel('J’ai payé').setStyle(ButtonStyle.Success))]});if(!ok)return smartReply(interaction, {content:'❌ Impossible d’envoyer le MP.',flags:MessageFlags.Ephemeral});o.status='Paiement envoyé';o.paymentStatus='Envoyé'}else if(action==='order_archive')o.status='Archivée';db.orders[id]=o;writeJson(DB_FILE,db);await saveOrderCard(guild,o);return smartReply(interaction, {content:`✅ Commande : ${o.status}.`,flags:MessageFlags.Ephemeral})}
      ...
      if(interaction.customId.startsWith('payment_')){const[action,id]=interaction.customId.split(':'),db=getDb(),p=db.payments[id];if(!p)return smartReply(interaction, {content:'❌ Paiement introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(p.guildId),c=getConfig(p.guildId),member=await getMember(guild,interaction.user.id);if(!isStaff(member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});if(action==='payment_accept'){const r=await validatePayment(guild,id,interaction.user.id);return smartReply(interaction, {content:r.already?'ℹ️ Paiement déjà validé, aucun doublon créé.':'✅ Paiement validé et parcours client exécuté.',flags:MessageFlags.Ephemeral})}if(action==='payment_refuse'){p.status='Refusé';db.payments[id]=p;writeJson(DB_FILE,db);await sendDm(p.userId,{content:`❌ Paiement ${id} refusé.`});return smartReply(interaction, {content:'✅ Paiement refusé.',flags:MessageFlags.Ephemeral})}if(action==='payment_newproof'){await sendDm(p.userId,{content:`⚠️ Une nouvelle preuve est demandée pour ${id}.`});return smartReply(interaction, {content:'✅ Demande envoyée.',flags:MessageFlags.Ephemeral})}if(action==='payment_contact'){const modal=new ModalBuilder().setCustomId(`contact_modal:payments:${id}`).setTitle('Contacter le client');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('text').setLabel('Message').setStyle(TextInputStyle.Paragraph).setRequired(true)));return interaction.showModal(modal)}}
      if(interaction.customId.startsWith('project_')){const[action,id]=interaction.customId.split(':'),db=getDb(),p=db.projects[id];if(!p)return smartReply(interaction, {content:'❌ Projet introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(p.guildId),c=getConfig(p.guildId),member=await getMember(guild,interaction.user.id);if(!isStaff(member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});if(action==='project_claim'){p.claimedBy=interaction.user.id;db.projects[id]=p;writeJson(DB_FILE,db);await saveProjectCards(guild,p);return smartReply(interaction, {content:'✅ Projet pris.',flags:MessageFlags.Ephemeral})}if(action==='project_prev'||action==='project_next'){const idx=action==='project_prev'?Math.max(0,p.stageIndex-1):Math.min(PROJECT_STAGES.length-1,p.stageIndex+1);const changed=await changeProjectStage(guild,p,idx);return smartReply(interaction, {content:changed?'✅ Étape modifiée.':'ℹ️ Étape inchangée.',flags:MessageFlags.Ephemeral})}if(action==='project_contact'){const modal=new ModalBuilder().setCustomId(`contact_modal:projects:${id}`).setTitle('Contacter le client');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('text').setLabel('Message').setStyle(TextInputStyle.Paragraph).setRequired(true)));return interaction.showModal(modal)}}
      if(interaction.customId.startsWith('test_status:')){const parts=interaction.customId.split(':'),id=parts[1],status=parts.slice(2).join(':'),db=getDb(),t=db.tests[id];if(!t)return smartReply(interaction, {content:'❌ Test introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(t.guildId),c=getConfig(t.guildId),member=await getMember(guild,interaction.user.id);if(interaction.user.id!==t.userId&&!isStaff(member,c))return smartReply(interaction, {content:'❌ Tu ne peux modifier que tes tests.',flags:MessageFlags.Ephemeral});t.status=status;db.tests[id]=t;writeJson(DB_FILE,db);await interaction.update({embeds:[testEmbed(t)],components:testControls(t)});await sendDm(t.userId,{embeds:[embed(`🧪 Mise à jour ${id}`,`Statut : **${status}**`)]});return}
      if(interaction.customId.startsWith('bug_status:')){const parts=interaction.customId.split(':'),id=parts[1],status=parts.slice(2).join(':'),db=getDb(),b=db.bugs[id];if(!b)return smartReply(interaction, {content:'❌ Bug introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(b.guildId),c=getConfig(b.guildId),member=await getMember(guild,interaction.user.id);if(interaction.user.id!==b.userId&&!isStaff(member,c))return smartReply(interaction, {content:'❌ Tu ne peux modifier que tes bugs.',flags:MessageFlags.Ephemeral});b.status=status;db.bugs[id]=b;writeJson(DB_FILE,db);await interaction.update({embeds:[bugEmbed(b)],components:bugControls(b)});await sendDm(b.userId,{embeds:[embed(`🐞 Mise à jour ${id}`,`Statut : **${status}**`)]});return}
      if(interaction.customId.startsWith('ticket_')){const[action,id]=interaction.customId.split(':'),db=getDb(),t=db.tickets[id];if(!t)return smartReply(interaction, {content:'❌ Ticket introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(t.guildId),c=getConfig(t.guildId),member=await getMember(guild,interaction.user.id);if(!isStaff(member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});const ch=await guild.channels.fetch(t.channelId).catch(()=>null);if(action==='ticket_claim'){t.claimedBy=interaction.user.id;db.tickets[id]=t;writeJson(DB_FILE,db);return smartReply(interaction, {content:`✅ Ticket pris par ${interaction.user}.`})}if(action==='ticket_contact'){const modal=new ModalBuilder().setCustomId(`contact_modal:tickets:${id}`).setTitle('Contacter le client');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('text').setLabel('Message').setStyle(TextInputStyle.Paragraph).setRequired(true)));return interaction.showModal(modal)}if(action==='ticket_member_add'||action==='ticket_member_remove'){return smartReply(interaction, {content:'Choisis un utilisateur :',components:[new ActionRowBuilder().addComponents(new UserSelectMenuBuilder().setCustomId(`${action}_select:${id}`).setPlaceholder('Sélectionner un utilisateur').setMinValues(1).setMaxValues(1))],flags:MessageFlags.Ephemeral})}if(action==='ticket_rename'){const modal=new ModalBuilder().setCustomId(`ticket_rename_modal:${id}`).setTitle('Renommer le ticket');modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('name').setLabel('Nouveau nom').setStyle(TextInputStyle.Short).setRequired(true)));return interaction.showModal(modal)}if(action==='ticket_close'){t.status='Fermé';db.tickets[id]=t;writeJson(DB_FILE,db);if(ch)await ch.permissionOverwrites.edit(t.userId,{SendMessages:false});return smartReply(interaction, {content:'🔒 Ticket fermé.'})}if(action==='ticket_reopen'){t.status='Ouvert';db.tickets[id]=t;writeJson(DB_FILE,db);if(ch)await ch.permissionOverwrites.edit(t.userId,{SendMessages:true});return smartReply(interaction, {content:'🔓 Ticket rouvert.'})}if(action==='ticket_archive'){t.status='Archivé';db.tickets[id]=t;writeJson(DB_FILE,db);if(ch)await ch.setName(`archive-${ch.name}`.slice(0,100));return smartReply(interaction, {content:'📁 Ticket archivé.'})}}
    }

    if(interaction.isUserSelectMenu()){
      const[action,id]=interaction.customId.split(':'),db=getDb(),t=db.tickets[id];if(!t)return smartReply(interaction, {content:'❌ Ticket introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(t.guildId),c=getConfig(t.guildId),member=await getMember(guild,interaction.user.id);if(!isStaff(member,c))return smartReply(interaction, {content:'❌ Réservé au personnel.',flags:MessageFlags.Ephemeral});const targetId=interaction.values[0],ch=await guild.channels.fetch(t.channelId).catch(()=>null);if(!ch)return smartReply(interaction, {content:'❌ Salon ticket introuvable.',flags:MessageFlags.Ephemeral});if(action==='ticket_member_add_select'){await ch.permissionOverwrites.edit(targetId,{ViewChannel:true,SendMessages:true,ReadMessageHistory:true});if(!t.memberIds.includes(targetId))t.memberIds.push(targetId)}else{await ch.permissionOverwrites.delete(targetId).catch(()=>{});t.memberIds=t.memberIds.filter(x=>x!==targetId)}db.tickets[id]=t;writeJson(DB_FILE,db);return interaction.update({content:'✅ Permissions du ticket mises à jour.',components:[]})
    }

    if(interaction.isModalSubmit()){
      const gid=interaction.guildId || null;
      if(interaction.customId==='quote_modal'){const q=await createQuote(interaction.guild,interaction.user.id,interaction.fields.getTextInputValue('project'),interaction.fields.getTextInputValue('service'),interaction.fields.getTextInputValue('description'));return smartReply(interaction, {content:`✅ Devis créé : **${q.id}**.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId.startsWith('order_modal:')){const offerKey=interaction.customId.split(':')[1],o=await createOrder(interaction.guild,interaction.user.id,offerKey,interaction.fields.getTextInputValue('project'),interaction.fields.getTextInputValue('service'),`${interaction.fields.getTextInputValue('description')}\nDélai souhaité : ${interaction.fields.getTextInputValue('deadline')||'Non précisé'}`);return smartReply(interaction, {content:`✅ Commande créée : **${o.id}**.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId.startsWith('quote_price_modal:')){const id=interaction.customId.split(':')[1],db=getDb(),q=db.quotes[id];if(!q)return smartReply(interaction, {content:'❌ Devis introuvable.',flags:MessageFlags.Ephemeral});const price=Number(interaction.fields.getTextInputValue('price').replace(',','.'));if(!Number.isFinite(price)||price<0)return smartReply(interaction, {content:'❌ Prix invalide.',flags:MessageFlags.Ephemeral});q.price=price;q.status='Prix défini';db.quotes[id]=q;writeJson(DB_FILE,db);const guild=await getGuild(q.guildId);await saveQuoteCard(guild,q);return smartReply(interaction, {content:`✅ Prix : **${price.toFixed(2)} €**.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId==='payment_modal'||interaction.customId.startsWith('payment_order_modal:')){const orderId=interaction.customId==='payment_modal'?interaction.fields.getTextInputValue('order').trim().toUpperCase():interaction.customId.split(':')[1],proof=interaction.fields.getTextInputValue('proof'),db=getDb(),o=db.orders[orderId];if(!o||o.userId!==interaction.user.id)return smartReply(interaction, {content:'❌ Commande introuvable ou non autorisée.',flags:MessageFlags.Ephemeral});const existing=Object.values(db.payments).find(x=>x.orderId===orderId&&x.status==='En attente');if(existing)return smartReply(interaction, {content:`ℹ️ Un paiement est déjà en attente : **${existing.id}**.`,flags:MessageFlags.Ephemeral});const paymentId=nextId('payments','PAY'),latest=getDb();latest.payments[paymentId]={id:paymentId,guildId:o.guildId,orderId,userId:interaction.user.id,proof,status:'En attente',processed:false,createdAt:new Date().toISOString()};latest.orders[orderId].paymentStatus='À vérifier';latest.orders[orderId].status='Paiement à vérifier';writeJson(DB_FILE,latest);const guild=await getGuild(o.guildId),c=getConfig(o.guildId),ch=await guild.channels.fetch(c.channels.paiements).catch(()=>null);if(!ch?.isTextBased())return smartReply(interaction, {content:'❌ Salon Paiements non configuré.',flags:MessageFlags.Ephemeral});await ch.send({embeds:[embed(`💳 ${paymentId}`,`Commande : **${orderId}**\nClient : <@${interaction.user.id}>\nMontant attendu : **${o.price==null?'Non défini':`${Number(o.price).toFixed(2)} €`}**\n\nPreuve / référence :\n${proof}`,0xF39C12)],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`payment_accept:${paymentId}`).setLabel('Valider').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId(`payment_refuse:${paymentId}`).setLabel('Refuser').setStyle(ButtonStyle.Danger),new ButtonBuilder().setCustomId(`payment_newproof:${paymentId}`).setLabel('Nouvelle preuve').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`payment_contact:${paymentId}`).setLabel('Contacter').setStyle(ButtonStyle.Secondary))]});await saveOrderCard(guild,latest.orders[orderId]);return smartReply(interaction, {content:`✅ Paiement déclaré : **${paymentId}**.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId==='review_modal'){const title=interaction.fields.getTextInputValue('title'),rating=Math.max(1,Math.min(5,parseInt(interaction.fields.getTextInputValue('rating'),10)||5)),message=interaction.fields.getTextInputValue('message'),id=nextId('reviews','AVIS'),db=getDb();db.reviews[id]={id,guildId:interaction.guild.id,userId:interaction.user.id,title,rating,message,createdAt:new Date().toISOString()};writeJson(DB_FILE,db);const c=getConfig(interaction.guild.id),ch=await interaction.guild.channels.fetch(c.channels.avis).catch(()=>null);if(ch?.isTextBased())await ch.send({embeds:[embed(`⭐ ${title}`,`${message}\n\nAuteur : ${interaction.user}\nNote : **${rating}/5**\nCommande vérifiée`,0xF1C40F)]});return smartReply(interaction, {content:'✅ Avis publié.',flags:MessageFlags.Ephemeral})}
      if(interaction.customId==='bot_showcase_modal'){const name=interaction.fields.getTextInputValue('name'),photo=interaction.fields.getTextInputValue('photo'),description=interaction.fields.getTextInputValue('description'),features=interaction.fields.getTextInputValue('features');if(/https?:\/\//i.test(description)||/https?:\/\//i.test(features))return smartReply(interaction, {content:'❌ Les liens sont interdits dans Vos bots.',flags:MessageFlags.Ephemeral});const e=embed(`🤖 ${name}`,`${description}\n\n**Fonctionnalités :**\n${features}\n\nPrésenté par ${interaction.user}`);if(photo&&/^https?:\/\//i.test(photo))e.setThumbnail(photo);return smartReply(interaction, {content:'✅ Présentation enregistrée.',flags:MessageFlags.Ephemeral}).then(async()=>{const c=getConfig(interaction.guild.id),ch=await interaction.guild.channels.fetch(c.channels.vos_bots).catch(()=>null);if(ch?.isTextBased())await ch.send({embeds:[e]})})}
      if(interaction.customId==='test_modal'){const projectId=interaction.fields.getTextInputValue('project').trim().toUpperCase(),db=getDb();if(!db.projects[projectId])return smartReply(interaction, {content:'❌ Projet introuvable.',flags:MessageFlags.Ephemeral});const id=nextId('tests','TEST'),t={id,guildId:interaction.guild.id,userId:interaction.user.id,projectId,version:interaction.fields.getTextInputValue('version'),objective:interaction.fields.getTextInputValue('objective'),method:interaction.fields.getTextInputValue('method'),result:'',conclusion:'',status:'À tester',createdAt:new Date().toISOString()};const latest=getDb();latest.tests[id]=t;writeJson(DB_FILE,latest);const c=getConfig(interaction.guild.id),ch=await interaction.guild.channels.fetch(c.channels.tests_dev).catch(()=>null);if(!ch?.isTextBased())return smartReply(interaction, {content:'❌ Salon Tests non configuré.',flags:MessageFlags.Ephemeral});await ch.send({embeds:[testEmbed(t)],components:testControls(t)});await sendDm(t.userId,{embeds:[embed(`🧪 Test ${id} commencé`,`Projet : **${projectId}**\nStatut : **À tester**`)]});return smartReply(interaction, {content:`✅ Test créé : **${id}**.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId==='bug_modal'){const projectId=interaction.fields.getTextInputValue('project').trim().toUpperCase(),db=getDb();if(!db.projects[projectId])return smartReply(interaction, {content:'❌ Projet introuvable.',flags:MessageFlags.Ephemeral});const id=nextId('bugs','BUG'),b={id,guildId:interaction.guild.id,userId:interaction.user.id,projectId,severity:interaction.fields.getTextInputValue('severity'),description:interaction.fields.getTextInputValue('description'),steps:interaction.fields.getTextInputValue('steps'),claimedBy:null,status:'Nouveau',createdAt:new Date().toISOString()};const latest=getDb();latest.bugs[id]=b;writeJson(DB_FILE,latest);const c=getConfig(interaction.guild.id),ch=await interaction.guild.channels.fetch(c.channels.bugs).catch(()=>null);if(!ch?.isTextBased())return smartReply(interaction, {content:'❌ Salon Bugs non configuré.',flags:MessageFlags.Ephemeral});await ch.send({embeds:[bugEmbed(b)],components:bugControls(b)});await sendDm(b.userId,{embeds:[embed(`🐞 Bug ${id} créé`,`Projet : **${projectId}**\nStatut : **Nouveau**`)]});return smartReply(interaction, {content:`✅ Bug créé : **${id}**.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId==='sav_modal'){const ref=interaction.fields.getTextInputValue('reference').trim().toUpperCase(),description=interaction.fields.getTextInputValue('description'),db=getDb();let project=db.projects[ref];if(!project&&db.orders[ref]?.projectId)project=db.projects[db.orders[ref].projectId];if(!project)return smartReply(interaction, {content:'❌ Projet ou commande introuvable.',flags:MessageFlags.Ephemeral});const delivery=Object.values(db.deliveries).find(x=>x.projectId===project.id);if(!delivery)return smartReply(interaction, {content:'❌ Aucune livraison enregistrée pour ce projet.',flags:MessageFlags.Ephemeral});const days=(Date.now()-new Date(delivery.createdAt).getTime())/86400000;if(days>14)return smartReply(interaction, {content:'❌ La garantie technique de 14 jours est expirée.',flags:MessageFlags.Ephemeral});const ch=await createTicket(interaction.guild,interaction.user.id,'sav',false,project.orderId);await ch.send({embeds:[embed('🛠️ Demande SAV',`Projet : **${project.id}**\n${description}`)]});return smartReply(interaction, {content:`✅ Ticket SAV créé : ${ch}.`,flags:MessageFlags.Ephemeral})}
      if(interaction.customId.startsWith('ticket_rename_modal:')){const id=interaction.customId.split(':')[1],db=getDb(),t=db.tickets[id];if(!t)return smartReply(interaction, {content:'❌ Ticket introuvable.',flags:MessageFlags.Ephemeral});const guild=await getGuild(t.guildId),ch=await guild.channels.fetch(t.channelId).catch(()=>null);if(!ch)return smartReply(interaction, {content:'❌ Salon introuvable.',flags:MessageFlags.Ephemeral});await ch.setName(interaction.fields.getTextInputValue('name').toLowerCase().replace(/[^a-z0-9-]/g,'-').slice(0,100));return smartReply(interaction, {content:'✅ Ticket renommé.',flags:MessageFlags.Ephemeral})}
      if(interaction.customId.startsWith('contact_modal:')){const[,collection,id]=interaction.customId.split(':'),db=getDb(),item=db[collection]?.[id];if(!item)return smartReply(interaction, {content:'❌ Élément introuvable.',flags:MessageFlags.Ephemeral});const ok=await sendDm(item.userId,{embeds:[embed(`💬 Message concernant ${id}`,interaction.fields.getTextInputValue('text'))]});return smartReply(interaction, {content:ok?'✅ Message envoyé.':'❌ Impossible d’envoyer le MP.',flags:MessageFlags.Ephemeral})}
    }
  }catch(error){const gid=interaction.guildId||null;await logError(gid,error,`Interaction ${interaction.type} ${interaction.customId||interaction.commandName||''}`);if(!interaction.replied&&!interaction.deferred)await smartReply(interaction, {content:`❌ Erreur : ${error.message||'Une erreur est survenue.'}`,flags:MessageFlags.Ephemeral}).catch(()=>{});}
});
