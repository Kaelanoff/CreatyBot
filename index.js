  ...
  try{
    ...
    if(interaction.isChatInputCommand()){
      ...
      if(interaction.commandName==='config'){
        ...
        if(sub==='role'){const type=interaction.options.getString('type'),role=interaction.options.getRole('role');const roleLabel=ROLES.find(([k])=>k===type)?.[1]||type;setConfig(guild.id,'roles',type,role.id);await journal(guild.id,`Configuration rôle ${roleLabel} (${type}) → ${role.id}`);if(['fondateur','cofondateur','directeur','directeur_general'].includes(type))await safeLog(guild.id,'acces_total',{content:`⚠️ Configuration sensible : rôle **${roleLabel}** modifié par <@${interaction.user.id}>.`});return interaction.reply({content:`✅ **${roleLabel}** configuré sur ${role}.`,flags:MessageFlags.Ephemeral})}
