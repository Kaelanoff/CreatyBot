...
const {
  ...
  ActivityType, UserSelectMenuBuilderrequire('dotenv').config();
...
async function safeMemberOverwrite(channel,userId,permissions){
  if(!channel?.guild||!userId)return false;

  const member=await channel.guild.members.fetch(userId).catch(()=>null);
  if(!member){
    console.warn(`⚠️ Permission ignorée : membre ${userId} introuvable sur ${channel.guild.name}.`);
    return false;
  }
  ...
  try{
    ...
    // Discord 10009 = l'overwrite visé n'existe/ne peut pas être résolu.
    // On retente en supprimant un ancien overwrite éventuel puis en le recréant.
    if(error?.code===10009){
...
async function safeDeleteMemberOverwrite(channel,userId){
  if(!channel?.guild||!userId)return false;
  const member=await channel.guild.members.fetch(userId).catch(()=>null);
  if(!member){
